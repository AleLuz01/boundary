...
func TestConnectTargetCli(t *testing.T) {
	...
	t.Cleanup(func() {
		output := e2e.RunCommand("boundary", e2e.WithArgs("host-catalogs", "delete", "-id", newHostCatalogId))
		require.NoError(t, output.Err, string(output.Stderr))
	})
	...
	t.Cleanup(func() {
		output := e2e.RunCommand("boundary", e2e.WithArgs("host-sets", "delete", "-id", newHostSetId))
		require.NoError(t, output.Err, string(output.Stderr))
	})
	...
	t.Cleanup(func() {
		output := e2e.RunCommand("boundary", e2e.WithArgs("hosts", "delete", "-id", newHostId))
		require.NoError(t, output.Err, string(output.Stderr))
	})
	t.Logf("Created Host: %s", newHostId)

	// Add host to host set
	output = e2e.RunCommand("boundary", e2e.WithArgs("host-sets", "add-hosts", "-id", newHostSetId, "-host", newHostId))
	require.NoError(t, output.Err, string(output.Stderr))
	...
	t.Cleanup(func() {
		output := e2e.RunCommand("boundary", e2e.WithArgs("targets", "delete", "-id", newTargetId))
		require.NoError(t, output.Err, string(output.Stderr))
	})
