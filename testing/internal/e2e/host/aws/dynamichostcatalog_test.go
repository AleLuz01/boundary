...
func TestCreateAwsDynamicHostCatalogCli(t *testing.T) {
	...
	output := e2e.RunCommand("boundary", e2e.WithArgs("host-catalogs", "create", "plugin",
	...
	// Create a host set
	output = e2e.RunCommand("boundary", e2e.WithArgs("host-sets", "create", "plugin",
	...
	err = e2e.Retry(t, 5, 3*time.Second, func() error {
...
func TestCreateAwsDynamicHostCatalogApi(t *testing.T) {
	...
	err = e2e.Retry(t, 10, 3*time.Second, func() error {
